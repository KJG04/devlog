...
tags:
  ...
  - react query
  - hook
keyword: react, graphql, react query, hook, graphql-request
published: true
...
export async function PrefetchedGraphQLQueryHydrationBoundary({
  ...
  await Promise.all(prefetchQueryList.map(prefetchGraphQLQuery))
...
import { PrefetchedGraphQLQueryHydrationBoundary } from '@/util/server'
import { COUNTRY } from '@/gql/country'
